...
import { PortfolioSummary, Holding, UserProfile, UserPersonaType } from "@/types";
...
import { PersonaSwitcherBar } from "@/dashboard/PersonaSwitcherBar";
import { GrowwMarketHub } from "@/dashboard/GrowwMarketHub";
...
export default function StandaloneDashboardPage() {
  ...
  // Load User Profile & Adaptive Tab Priorities
  useEffect(() => {
    const savedUserStr = localStorage.getItem("niveshdristi_user");
    if (savedUserStr) {
      try {
        const u: UserProfile = JSON.parse(savedUserStr);
        setCurrentUser(u);

        // Adapt default view based on user persona type
        if (u.user_type === "PRO_TRADER") {
          setActiveTab("screener");
        } else if (u.user_type === "WEALTH_MANAGER") {
          setActiveTab("intelligence");
          setProSubTab("rebalance");
        } else if (u.user_type === "INSTITUTIONAL_ANALYST") {
          setActiveTab("intelligence");
          setProSubTab("accuracy");
        } else {
      ...
      // Default fallback if visiting /dashboard directly without logging in
  ...
  const handleSwitchPersona = async (type: UserPersonaType) => {
    setLoading(true);
    try {
      const res = await demoLogin(type);
      setCurrentUser(res.user);
      localStorage.setItem("niveshdristi_user", JSON.stringify(res.user));
      localStorage.setItem("niveshdristi_token", res.access_token);
      showToast(`Switched persona to ${res.user.full_name} (${res.user.user_type.replace("_", " ")})`);
      
      // Adapt active tab
      if (type === "PRO_TRADER") setActiveTab("screener");
      else if (type === "WEALTH_MANAGER") { setActiveTab("intelligence"); setProSubTab("rebalance"); }
      else if (type === "INSTITUTIONAL_ANALYST") { setActiveTab("intelligence"); setProSubTab("accuracy"); }
      else setActiveTab("portfolio");

      loadData();
    } catch (err) {
      console.error(err);
      showToast("Failed to switch user persona.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleLogout = () => {
    localStorage.removeItem("niveshdristi_user");
    localStorage.removeItem("niveshdristi_token");
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 text-slate-900 selection:bg-emerald-100 selection:text-emerald-900">
      ...
      <Navbar
        ...
        brokerConnected={currentUser?.broker_connected || summary?.broker_connected || settings.broker}
        riskScore={currentUser?.risk_score || settings.riskScore}
        onRefresh={loadData}
        onOpenAddModal={() => setIsAddModalOpen(true)}
        theme={settings.theme}
        onToggleTheme={() => handleUpdateSettings({ theme: settings.theme === "light" ? "dark" : "light" })}
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        {activeTab === "portfolio" && (
          <div className="space-y-6">
            ...
            {/* Groww Style Financial Products & Asset Discovery Hub */}
            <div className="pt-6">
              <GrowwMarketHub
                onOpenStockDetail={(t) => setSelectedTickerForDetail(t)}
                onOpenTechnicalDrawer={(t) => setSelectedTickerForDrawer(t)}
      ...
      {/* Floating AI Advisor trigger button */}
      {activeTab !== "advisor" && (
        <button
      ...
      <AiStockAnalystModal
        ...
        onOpenAiChat={(p) => {
          if (p) setAdvisorInitialPrompt(p);
